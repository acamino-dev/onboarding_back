...
describe('saveS3Key integration', () => {
  it('persists s3Key on the KYC record', async () => {
    await saveS3Key(TEST_CREDIT_ID, TEST_S3_KEY, TABLE_NAME)
    ...
    expect(result.Item?.s3Key).toBe(TEST_S3_KEY)
  })

  it('throws when table name is invalid', async () => {
    await expect(saveS3Key(TEST_CREDIT_ID, TEST_S3_KEY, 'nonexistent-table')).rejects.toThrow(
      /Error on saveS3Key/
    )
  })
})
