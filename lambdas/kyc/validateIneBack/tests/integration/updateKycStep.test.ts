...
describe('updateKycStep integration', () => {
  it('updates step to ADDRESS in DynamoDB', async () => {
    await updateKycStep(TEST_CREDIT_ID, 'ADDRESS', TABLE_NAME)
    ...
    expect(result.Item?.['step']).toBe('ADDRESS')
  })

  it('throws wrapped Error when table does not exist', async () => {
    await expect(
      updateKycStep(TEST_CREDIT_ID, 'ADDRESS', 'nonexistent-table')
