...
describe('getKycByUserId integration', () => {
  it('returns KYC record with s3Keys and address for existing userId', async () => {
    ...
    expect(result.s3Keys).toEqual(READ_KYC_RECORD.s3Keys)
    expect(result.address).toBe(READ_KYC_RECORD.address)
  })
