...
import { TEST_CREDIT_ID, TEST_INE_DATA, TEST_KYC_RECORD, TEST_USER_ID } from './helpers/constants'
...
describe('updateKycWithIneData integration', () => {
  it('updates step to INE_FRONT and persists INE fields', async () => {
    await updateKycWithIneData(TEST_CREDIT_ID, TEST_INE_DATA, TABLE_NAME)
    ...
    expect(result.Item?.['domicilio']).toBe(TEST_INE_DATA.domicilio)
  })

  it('throws wrapped Error when creditId does not exist', async () => {
    await expect(
      updateKycWithIneData('nonexistent-credit-id', TEST_INE_DATA, 'nonexistent-table')
